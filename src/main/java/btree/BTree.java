...
public class BTree<K extends Comparable, V>
{
    ...
    public List<K> getKeySet(BTIteratorIF<K, V> iterImpl) throws IOException{
        list(iterImpl, false);
        return keySet;
    }
    ...
    //
    //public void list(BTIteratorIF<K, V> iterImpl, BufferedWriter tableWriter) throws IOException{
    public void list(BTIteratorIF<K, V> iterImpl, boolean print) throws IOException{
        if (mSize < 1) {
            return;
        }
        ...
        listEntriesInOrder(mRoot, iterImpl, print);
    }
    ...
    // Return false if it is done
    //
    //private boolean listEntriesInOrder(BTNode<K, V> treeNode, BTIteratorIF<K, V> iterImpl,  BufferedWriter tableWriter) throws IOException {
    private boolean listEntriesInOrder(BTNode<K, V> treeNode, BTIteratorIF<K, V> iterImpl, boolean print) throws IOException {
        if ((treeNode == null) ||
        ...
        for (int i = 0; i < currentKeyNum; ++i) {
            //listEntriesInOrder(BTNode.getLeftChildAtIndex(treeNode, i), iterImpl, tableWriter);
            listEntriesInOrder(BTNode.getLeftChildAtIndex(treeNode, i), iterImpl, print);

            keyVal = treeNode.mKeys[i];
            // TODO test
            if(print)
                System.out.println("key: " + keyVal.mKey + " Entries: " + keyVal.mValue);
            else
                this.keySet.add(keyVal.mKey);
            ...
            if (i == currentKeyNum - 1) {
               // listEntriesInOrder(BTNode.getRightChildAtIndex(treeNode, i), iterImpl,  tableWriter);
                listEntriesInOrder(BTNode.getRightChildAtIndex(treeNode, i), iterImpl, print);
            }
        }
